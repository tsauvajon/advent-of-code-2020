...
def solution():
    r = r'([0-9]+)\-([0-9]+)\ ([a-zA-Z0-9]+)\:\ ([a-zA-Z0-9]+)'
    def parse_line(line: str):
        matches = re.findall(r, line)[0]
        return matches

    def is_valid(line: str) -> bool:
        first_position, second_position, policy, password = parse_line(line)
        first_index = int(first_position)-1
        second_index = int(second_position)-1
        # xor
        return (password[first_index] == policy) != (password[second_index] == policy)
